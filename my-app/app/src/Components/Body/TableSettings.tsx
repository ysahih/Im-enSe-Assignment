...
const TableSettings = () => {
  ...
  const [checkedColumns, setCheckedColumns] = useState<Set<string>>(
    new Set(columns.map((column) => column.label))
  );
  ...
  const removeColumn = (label: string) => {
    context?.setColumnsState((prevColumns) =>
      prevColumns.filter((column) => column.label !== label)
    );
  };

  // here is where I add the column to the context or remove it, depending on the checkbox state
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <th className="p-2 h-[149px]  cursor-pointer">
          <RiSettings5Fill className="hover:text-[#42A4DF] m-auto size-[20px]" />
        </th>
      </DropdownMenuTrigger>

      <DropdownMenuContent className="bg-white h-[] rounded-[5px] border border-[#DCDCDC]">
        <FormGroup className="p-2">
          <h1 className="text-xs font-medium text-[#989797]">
          ...
          {columns.map((column) => (
            <div key={column.label} className="flex items-center py-2 gap-2">
              ...
              />
              <label htmlFor={column.label} className="text-[#313131] text-xs">
                {column.label}
              </label>
            </div>
          ))}
