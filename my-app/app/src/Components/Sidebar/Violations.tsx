  ...
  return (
    <div
      className={`relative inline-block flex items-center justify-center px-2 gap-2 h-10 cursor-pointer rounded-[5px]  text-[#C4C4C4] ${
        isOpen ? "text-white bg-[#42A4DF]" : ""
      }`}
      onMouseEnter={() => setShowPopup(true)}
      onMouseLeave={() => setShowPopup(false)}
    >
      <MdFrontHand
        className={`size-[12px] rounded-full outline outline-[1px] outline-[#42A4DF] outline-offset-2 text-[#42A4DF] ${
          isOpen ? "ml-1 text-[white] outline-white" : "hover:text-[#42A4DF]"
        }`}
      />
      {showPopup && !isOpen && (
      ...
      <span
        className={`flex items-center transition-width duration-200 whitespace-nowrap text-white ${
          isOpen ? "h-full w-full transition-none" : "hidden"
        }`}
      >
        P.P.E Violarions
      </span>
